...
const KEYS = COORDINATE_KEYS.concat(ROAD_ADDRESS_KEYS).concat(STREET_ADDRESS_KEYS);
const LOCALIZED = {
  address: {
    plural: "tieosoitteet",
    singular: "tieosoite"
...
function addConvertedValues(values, headerKeys) {
  ...
  if (roadAddresses) return addCoordinates(values).then(addStreetAddresses);
  if (streetAddresses) return addGeocodedCoordinates(values).then(addRoadAddresses);
...
function addStreetAddresses(values) {
  ...
  return Promise.map(values, reverseGeocode, { concurrency: CONCURRENT_REQUEST_LIMIT })
    .map(R.compose(R.pick(STREET_ADDRESS_KEYS), parseJSON))
