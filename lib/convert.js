...
const API_URL = "https://testijulkinen.vayla.fi";
const VKM_URL = API_URL + "/viitekehysmuunnin/muunna";

const POINT_HEADERS = ["X", "Y", "Tie", "Ajorata", "Tieosa", "Etäisyys", "Katunimi", "Katunumero", "Kunta", "Ely", "Urakka-alue", "Tunniste"];
const POINT_DAYS_HEADERS = ["Tie", "Ajorata", "Tieosa", "Etäisyys", "Tilannepvm", "Kohdepvm", "Tie output", "Ajorata output", "Tieosa output", "Etäisyys output", "Tunniste"];
const INTERVAL_HEADERS = ["Tie", "Ajorata", "Tieosa (alkupiste)", "Etäisyys (alkupiste)", "Tieosa (loppupiste)", "Etäisyys (loppupiste)", "AlkuX", "AlkuY", "LoppuX", "LoppuY", "Tunniste"];
...
const CONTROL_KEYS = [IDENTIFIER_KEY];

const ERROR_KEYS = ["valid", "error"].concat(EXTERNAL_ERROR_KEYS);
const POINT_KEYS = COORDINATE_KEYS.concat(ROAD_ADDRESS_KEYS).concat(STREET_ADDRESS_KEYS).concat(OTHER_ADDRESS_KEYS).concat(CONTROL_KEYS);
const INTERVAL_KEYS = INTERVAL_ROAD_ADDRESS_KEYS.concat(INTERVAL_COORDINATE_KEYS).concat(CONTROL_KEYS);
const POINT_DAYS_KEYS = ROAD_ADDRESS_KEYS.concat(OTHER_DAYS_KEYS).concat(ROAD_ADDRESS_KEYS_OUT).concat(CONTROL_KEYS);
...
function determineType(headerKeys) {
  if (headerKeys.includes(IDENTIFIER_KEY)) {
    var index = headerKeys.indexOf(IDENTIFIER_KEY);
...
function addCoordinates(addresses) {
  return decorateWith(GEOJSON_KEYS.structure, addresses, POINT_KEYS);
}


function addIntervalCoordinates(addresses) {
  return decorateWith(GEOJSON_KEYS.structure, addresses, INTERVAL_KEYS);
}
...
function decorateWith(jsonStructure, values, whitelistedKeys) {
  const data = {
    json: JSONtoValidString(values)
  };

  return httpPost(VKM_URL, data).then(R.pipe(
        R.propOr(values, jsonStructure.main),
        R.map(R.prop(jsonStructure.props)),
...
function JSONtoValidString(values) {
  ...
  str = str.replace(/"ajorata":0/g,'"ajorata":"0"');
  str = str.replace(/"ajorata":1/g,'"ajorata":"1"');
  str = str.replace(/"ajorata":2/g,'"ajorata":"2"');
